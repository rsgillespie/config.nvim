...
For general nvim help
  - :help

For info on how nvim uses lua
  - :help lua-guide

If you experience errors, run `:checkhealth` for more info.
...
do
  ...
  -- Set filetypes
  vim.filetype.add { extension = { pyt = 'python' } }
...
do
  ...
  vim.keymap.set({ 'n', 'v' }, '<leader>s*', builtin.grep_string, { desc = '[S]earch current word ("*")' })
  ...
  vim.keymap.set('n', '<leader>sw', function() builtin.live_grep { cwd = '~/vimwiki/' } end, { desc = '[S]earch by Grep in [W]iki' })
end
...
do
  ...
  require('blink.cmp').setup {
    ...
    sources = {
      providers = {
        -- prevent lsp provider from turning off buffer completions
        lsp = { fallbacks = {} },
        -- downrank buffer completions below snippets (-3 by default)
...
do
  ...
  local wiki = '/mnt/d/OneDrive - DOI/vimwiki'
  if vim.loop.os_uname().sysname == 'Windows_NT' then wiki = 'D:/OneDrive - DOI/vimwiki' end
  require('obsidian').setup {
    workspaces = {
      { name = 'vimwiki', path = wiki },
    },
    completion = {
      nvim_cmp = false,
